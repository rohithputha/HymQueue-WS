...
public class SubscriberCallback implements SubscriberFunction {
    ...
    public SubscriberCallback(String ip, int port, String commandId, BinaryProtocol binaryProtocol){
        this.ip = ip;
        this.port = port;
    ...
    public void consume(HymOutput hymOutput) {
        try {
            ...
            ArrayList hOutputList = new ArrayList<>(Arrays.asList(this.commandId,hymOutput.toString()));
