...
public class AddSubscriberCommandExecutor<V> implements CommandExecutor<V> {
    ...
    public List<String> execute(List<V> command, HymQueue hymQueue) {
        try{
            hymQueue.addSubscriber((String)command.get(2),(String)command.get(3),new SubscriberCallback((String)command.get(4),Integer.valueOf((String)command.get(5)), getCommandId(command), IBinaryProtocol.getInstance()));
